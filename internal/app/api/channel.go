...
func (c *Channel) CreateChannel(channel ChannelResult) (bool, error) {
	...
	if err != nil {
		logger.Errorf(`Error while creating channel %s: %s {"correlationId":"%s"}`, channel.Name, err.Error(), c.CorrelationID)
		return false, fmt.Errorf("Error while creating channel %s", channel.Name)
	}

	if exists {
		logger.Warningf(`Trying to create existent channel %s {"correlationId":"%s"}`, channel.Name, c.CorrelationID)
	...
	if err != nil {
		logger.Errorf(`Something wrong with channel %s data: %s {"correlationId":"%s"}`, channel.Name, err.Error(), c.CorrelationID)
...
func (c *Channel) GetChannelByName(name string) (ChannelResult, error) {
	...
	if err != nil {
		logger.Errorf(`Error while getting channel %s: %s {"correlationId":"%s"}`, name, err.Error(), c.CorrelationID)
		return channelResult, fmt.Errorf("Error while getting channel %s", name)
	}

	if !exists {
		logger.Warningf(`Trying to get non existent channel %s {"correlationId":"%s"}`, name, c.CorrelationID)
...
func (c *Channel) UpdateChannelByName(channel ChannelResult) (bool, error) {
	...
	if err != nil {
		logger.Errorf(`Error while updating channel %s: %s {"correlationId":"%s"}`, channel.Name, err.Error(), c.CorrelationID)
		return false, fmt.Errorf("Error while updating channel %s", channel.Name)
	}

	if !exists {
		logger.Warningf(`Trying to create non existent channel %s {"correlationId":"%s"}`, channel.Name, c.CorrelationID)
...
func (c *Channel) DeleteChannelByName(name string) (bool, error) {
	...
	if err != nil {
		logger.Errorf(`Error while deleting channel %s: %s {"correlationId":"%s"}`, name, err.Error(), c.CorrelationID)
		return false, fmt.Errorf("Error while deleting channel %s", name)
	}

	if deleted <= 0 {
		logger.Warningf(`Trying to delete non existent channel %s {"correlationId":"%s"}`, name, c.CorrelationID)
